...
module.exports = class PetController {
    ...
    static async removePetById(req, res) {
        ...
        if(!ObjectId.isValid(id)) {
            res.status(422).json({ message: "ID inváido" })
            return
        }
        ...
        if(pet.user._id.toString() !== user._id.toString()){
            res.status(422).json({ message: "Houve um problema em processa a sua solicitação!"})
            return
        }
    ...
    static async concludeAdoption(req, res) {
        ...
        res.status(200).json({
            message: 'Parabéns! O ciclo de adoçao foi finalizado com sucesso!'
        })
